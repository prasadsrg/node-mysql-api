  ...
  async filter(reqData: any) {
    try {
      ...
      if (reqData.session.role == "SuperAdmin") {
        ...
        let profileData: any = await this.vehicleDao.searchByBranch(
  ...
  async save(item) {
    try {
      if (await this.validate(item)) {
        ...
        let vehcileRegistrationData: any = await this.vrService.save(
        ...
        let vehcileInsurenceData: any = await this.viService.save(
