...
export class AuthService {
    ...
    async reteriveProfileDetails(userId: string) {
        try {
            ...
            if (accountObj != null) {
                ...
                var menuAccessObj = await this.appMenuDAO.search({ role: accountObj.role, active:accountObj.active});
    ...
    async sendSocailResponse(reqData: any) {
        try {
            var responseData: any = {};
            var profileObj: any = await this.profileDAO.search({ email: reqData.userId});
